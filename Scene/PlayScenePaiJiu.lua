...
local PlayScenePaiJiu = class("PlayScenePaiJiu", GameBase)

PlayScenePaiJiu.__index = PlayScenePaiJiu

--[[

--]]

PlayScenePaiJiu.FLIMTYPE = {
	FLIMLAYER_BAR				= 1,
	FLIMLAYER_BU				= 2,
}
...
function PlayScenePaiJiu:ctor(enterRoomMsgTbl,Roomname,ChoseGameid)
    GameBase:ctor()
    GameBase:initdata()
...
--    venue_close:setLocalZOrder(100005)
    ...
    self.PukeSprite = {}
    for i = 1 , 10 do 
        self.PukeBack[i] = cc.Sprite:create("res/majiang/MaJBack.png")
        self.PukeBack[i]:setPosition(cc.p(850,1050))
        self.PukeBack[i]:setTag(i+10)   --11-20
        self.rootNode:addChild(self.PukeBack[i],99)

        self.PukeSprite[i] = cc.Sprite:create("res/majiang/MaJBack.png")
...
-- 进入游戏房间
function PlayScenePaiJiu:enterGameRoom(sender)
    --gt.Gameroomid = 0
    --gt.SelectroomSite = 0
    --gt.SelectroomId = 0
...
function PlayScenePaiJiu:onchipClicked(butobj,tap)
    self.betcursor:setPositionX(butobj:getPositionX())
    self.chipma = tap
end

function PlayScenePaiJiu:onJettonAreaClicked(sender, eventType)
    --加限额判断  总额-减投注
        local animation = cc.RotateTo:create(0.2, 180)
        self.venue_pop:runAction(cc.RepeatForever:create(animation))
...
-- 先期收回赢家币
function PlayScenePaiJiu:HiuShuWinChip()
...
function PlayScenePaiJiu:HiuShumovechip(WinIndx,Indx)
...
-- 回收所有币
function PlayScenePaiJiu:HiuShuChip()
    --cc.Sequence:create(cc.DelayTime:create(8),self:OnsetSettle())
    ...
    for index = 0, self.chipTap do 
        ...
        --gt.log("chipTapnum==",self.delindex*1000 + index)
        if TChipSprite then
            ...
            local spa = cc.Sequence:create(moveTo, cc.DelayTime:create(0.8),call1)
...
function PlayScenePaiJiu:OnplaygameNode()
    
end

--发送option消息
function PlayScenePaiJiu:onReady()

end

function PlayScenePaiJiu:onNodeEvent(eventName)
...
function PlayScenePaiJiu:onTouchBegan(touch, event)
...
function PlayScenePaiJiu:onTouchMove(touch, event)
    self.MovePos = true
    return true
end
function PlayScenePaiJiu:onTouchEnd(touch, event)
        local animation = cc.RotateTo:create(0.2, 180)
        self.venue_pop:runAction(cc.RepeatForever:create(animation))
...
function PlayScenePaiJiu:PfbMove(bVisible)
    Utils.setClickEffect()  
    local pos
    if bVisible then
...
function PlayScenePaiJiu:update(delta)	
    self:UpdateOpenTime()
...
--            end 
            ...
            for i = 1 , 5 do 
                ...
                --self.result_num[i]:setVisible(false)
                self.SprFullAnimal[i]:setVisible(false)
            end
            self:PukeResult(false)  --显示点数结果消失
            --self:HiuShuChip()
...
--买满提示框
function PlayScenePaiJiu:InitBuyfull()
...
function PlayScenePaiJiu:OnGameBuyFull(code,msg_id,bet_money)
    if code == 1 then
        require("app/views/UI/NoticeTips"):create("提示","买满失败，系统错误！", nil, nil, true)
    elseif code == 2 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，元宝不足！",nil, nil, true)
    elseif code == 3 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，超期！",nil, nil, true)
    elseif code == 4 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，已达投注区上限！",nil, nil, true)
    elseif code == 5 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，投注区无效！",nil, nil, true)
    elseif code == 6 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，玩法不支持（不支持买满）！",nil, nil, true)
    elseif code == 7 then 
        require("app/views/UI/NoticeTips"):create("提示", "买满失败，非买满时间！",nil, nil, true)
    elseif code == 0 then
...
function PlayScenePaiJiu:faPuke()
        local PukeBack = self.rootNode:getChildByTag(self.PukeNum)
        --gt.soundEngine:playEffect("ShowCard",false)
        PukeBack:setVisible(true)
        
        local pos1 = cc.p(275,1050)
        local moveTo1 = cc.MoveTo:create(0.6, pos1)
        local spa1 = cc.Spawn:create(moveTo1)
        local Seq = cc.Sequence:create(spa1,nil)

        local PukeBack2 = self.rootNode:getChildByTag(self.PukeNum+1)
        PukeBack2:setVisible(true)
        
        local pos2 = cc.p(475,1050)
        local moveTo2 = cc.MoveTo:create(0.6, pos2)
        local spa2 = cc.Spawn:create(moveTo2)

        local call2 = cc.CallFunc:create(function ()
            self:showPai(self.PukeNum)
	    end)
        local Seq2 = cc.Sequence:create(spa2,call2)
	    PukeBack2:stopAllActions()
	    PukeBack2:runAction(Seq2)
	    PukeBack:runAction(Seq)
end

function PlayScenePaiJiu:showPai(PukeNum)
    ...
    local paiSpZ = self.rootNode:getChildByTag(PukeNum +10)
    paiSpZ:setTexture("res/majiang/Tong"..MsgTbl[PukeNum-10]..".png")
    paiSpZ:setPosition(paiSp:getPosition())
    --paiSp:setScaleX(0)
    --paiSpZ:setVisible(true)
    paiSpZ:setScaleX(0.2)
    paiSpZ:setScaleY(1)
    ...
    local call = cc.CallFunc:create(function ()
        ...
        paiSpZ:setVisible(true)
        paiSpZ:runAction(cc.Sequence:create(cc.ScaleTo:create(0.4,1,1),null))        
    end)
    paiSp:runAction(cc.Sequence:create(cc.ScaleTo:create(0.4,0.2,1),call))
         
    local paiSp2 = self.rootNode:getChildByTag(PukeNum+1)
    local paiSpZ2 = self.rootNode:getChildByTag(PukeNum +11)
    paiSpZ2:setTexture("res/majiang/Tong"..MsgTbl[PukeNum-9]..".png")
    paiSpZ2:setPosition(paiSp2:getPosition())
    --paiSp:setScaleX(0)
    --paiSpZ:setVisible(true)
    paiSpZ2:setScaleX(0.2)
    paiSpZ2:setScaleY(1)
    paiSpZ2:setRotation(360)
    paiSpZ2:setOpacity(255)
    ...
    local call2 = cc.CallFunc:create(function ()
        ...
        paiSp2:setVisible(false)
        paiSpZ2:setVisible(true)
        paiSpZ2:runAction(cc.Sequence:create(cc.ScaleTo:create(0.4,1,1),call3))
        
    end)
    paiSp2:runAction(cc.Sequence:create(cc.ScaleTo:create(0.4,0.2,1),call2))
end

function PlayScenePaiJiu:MovePukePai()
        
        local MaJNum1 = self.PukeNum + 10
        local paiSpZ = self.rootNode:getChildByTag(MaJNum1)
        local paiSpZ2 = self.rootNode:getChildByTag(MaJNum1+1)
            ...
            if MaJNum1 == 21 then
                pos1 = cc.p(305,620)
                pos2 = cc.p(375,620)
            elseif MaJNum1 == 23 then
                pos1 = cc.p(550,517)
                pos2 = cc.p(620,517)
            elseif MaJNum1 == 25 then
                pos1 = cc.p(427.5,275)
                pos2 = cc.p(497.5,275)
            elseif MaJNum1 == 27 then
                pos1 = cc.p(182.5,275)
                pos2 = cc.p(252.5,275)
            elseif MaJNum1 == 29 then
                pos1 = cc.p(60,517)
                pos2 = cc.p(130,517)
            end
            local moveTo1 = cc.MoveTo:create(0.2, pos1)
            local ScaleTo1 = cc.ScaleTo:create(0.2,0.35)
            ...
            local ScaleTo2 = cc.ScaleTo:create(0.8,0.35)
            local spa2 = cc.Spawn:create(moveTo2,ScaleTo2)

            local call2 = cc.CallFunc:create(function ()
            
                self.PukeNum = self.PukeNum +2
                if self.PukeNum < 20 then
                    self:faPuke()
                else
            ...
            local Seq2 = cc.Sequence:create(spa2,call2)
	        paiSpZ2:stopAllActions()
	        paiSpZ2:runAction(Seq2)
	        paiSpZ:runAction(Seq)
       
end

function PlayScenePaiJiu:OnSetLotteryNo(lottery_no)
    self.CartrackGame:SetLotteryNo(true)
end

function PlayScenePaiJiu:PukeResult(bVisible)
    local MsgTbl = gt.string_split(GameBase.RoomMsgTbl[2],",")
    for i = 1, 10  do
        if MsgTbl[i] == "10" then
            MsgTbl[i] = "0"
    ...
    for i=1,5 do
        ...
        --self.chipLayer:addChild(paiSpNd,2)
    end
    --table.sort(SortNum)
    --dump(SortNum)
--    for i = 1 ,5 do
--        for j = 1 , 5 do
--            if num[i] == SortNum[j] then
--                BigNum[i] = j
--            end
--        end
--    end
...
function PlayScenePaiJiu:UpdateGameItem(tag, cellData)
...
function PlayScenePaiJiu:InitGameScene()
    
end
function PlayScenePaiJiu:InitchipTap()
    --选
    self.chipma = 1
...
--投注结算广播
function PlayScenePaiJiu:OnsetSettle()
...
function PlayScenePaiJiu:Onaddchip(area_id,chip,issend)
...
function PlayScenePaiJiu:MoveChip(area_id,chipid,issend)
    ...
    print("chipSprite:setTag(chipTapnum + self.chipTap)",chipTapnum + self.chipTap)
...
--创建输区域的筹码
function PlayScenePaiJiu:CreatChip(chipid,lose_id)
...
function PlayScenePaiJiu:CreatChip2(chipid,lose_id)
    local str = string.format("res/ChouMa/chip%d.png", chipid)
...
--移动输区域的筹码
function PlayScenePaiJiu:LoseToWin(move_id,area_id,chipid,bMove)
    local chipNum = chipid
    for i=1 ,5 do
        if chipNum == 0 then
            break
...
function PlayScenePaiJiu:MoveWinChip(move_id,area_id,chipid,bMove,nChip)
    local str = string.format("res/ChouMa/chip%d.png", chipid)
...
function PlayScenePaiJiu:MoveWinToHome(area_id,nDelayTime)
...
function PlayScenePaiJiu:OnShowaddchip(area_id,chipid,isfull)
    ...
    --gt.log("OnShowaddchip =====",area_id,chipid,isfull)
     if isfull==1 then
        self.SprFullAnimal[area_id+1]:setVisible(true)
     end
end

function PlayScenePaiJiu:OnSetTouzhuchip(chipid)
    self.Touzhu_num:setString(chipid)
end
-- 断线重连,初始化数据
function PlayScenePaiJiu:reLogin()
    -- 进游戏桌子
    self.loadingtime = 0
    ...
    local playScene = require("app/views/Scene/PlayScenePaiJiu"):create(self.roomInfo,GameBase.RoomMsgTbl[4],gt.Gameroomid)
    cc.Director:getInstance():replaceScene(playScene)
...
function PlayScenePaiJiu:OnBetSettleResp(Lorder)
--    self.CartrackGame:StopCarBao()
--    self.gametap:showzhonjian(GameBase.RoomMsgTbl)
