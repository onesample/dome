...
local PlaySceneJC = class("PlaySceneJC", GameBase)

PlaySceneJC.__index = PlaySceneJC
...
function PlaySceneJC:ctor(enterRoomMsgTbl,Roomname,ChoseGameid)
    GameBase:ctor()
    GameBase:initdata()
    --保存房间信息20161125
...
--	        end)
    ...
    self.NchangGame = require("app/views/UI/Nongchang"):create(GameBase.RoomMsgTbl)
...
-- 进入游戏房间
function PlaySceneJC:enterGameRoom(sender)
        --gt.Gameroomid = 0
        --gt.SelectroomSite = 0
        --gt.SelectroomId = 0
...
function PlaySceneJC:onchipClicked(butobj,tap)
    self.betcursor:setPositionX(butobj:getPositionX())
    self.chipma = tap
end

function PlaySceneJC:onJettonAreaClicked(sender, eventType)
    --加限额判断  总额-减投注 
...
function PlaySceneJC:HiuShuChip()
...
function PlaySceneJC:OnplaygameNode()
    

end

--发送option消息
function PlaySceneJC:onReady()

end

function PlaySceneJC:onNodeEvent(eventName)
...
function PlaySceneJC:onTouchBegan(touch, event)
...
function PlaySceneJC:onTouchMove(touch, event)
    self.MovePos = true
    return true
end
function PlaySceneJC:onTouchEnd(touch, event)
        local animation = cc.RotateTo:create(0.2, 180)
        self.venue_pop:runAction(cc.RepeatForever:create(animation))
...
function PlaySceneJC:PfbMove(bVisible)
    Utils.setClickEffect()  
    local pos
    if bVisible then
...
function PlaySceneJC:OnSetLotteryNo(lottery_no)
    --self.CartrackGame:SetLotteryNo(lottery_no)
end

function PlaySceneJC:update(delta)	
    self:UpdateOpenTime()
...
function PlaySceneJC:UpdateGameItem(tag, cellData)
...
function PlaySceneJC:InitGameScene()
    
end
function PlaySceneJC:InitchipTap()
    --选
    self.chipma = 1
...
--投注结算广播
function PlaySceneJC:OnsetSettle()
    if #self.ranking < 1 then
        return
    end
    local ProfitScene =  self:getChildByTag(1002)
    if ProfitScene then
        ProfitScene:UdatCellItem(earnings,ranking)
    else
        ProfitScene = require("app/views/Scene/ProfitScene"):create(earnings,ranking)
        self:addChild(ProfitScene,100,1002)
    end
end

--同步加币效果  区域：area_id,  要加的金币：chip
function PlaySceneJC:Onaddchip(area_id,chip,issend)
    if chip > 0 then
        gt.soundEngine:playEffect("ChipsClick",false)
    end
...
function PlaySceneJC:MoveChip(area_id,chipid,issend)
    ...
    if self.ClickArea > 4 or area_id > 4 then
        ...
        pos = cc.p(bet_box:getPositionX()+ math.random(-100,100), bet_box:getPositionY()+ math.random(-70,60))
    end
...
function PlaySceneJC:reLogin()
    ...
    local playScene = require("app/views/Scene/PlaySceneJC"):create(self.roomInfo,GameBase.RoomMsgTbl[4],gt.Gameroomid)
    cc.Director:getInstance():replaceScene(playScene)
...
function PlaySceneJC:OnBetSettleResp()
...
--function PlaySceneJC:getBetDetailByRoom()
